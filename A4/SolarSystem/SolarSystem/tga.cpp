...
#pragma pack(1)
//pack的目的是为了使得shorts和char成序列存储，能够直接从文件中读取
struct TGAHeader//18 bytes
{
    char id_length;
    char map_type;
    char type;
...
TGA::TGA(char* imagePath)
{
    ...
    //column counter, row counter, i & j lop counters and bytes per pixel
    ...
    //header type 2 is umcompressed RGB data without a color map/pallette
    if (header.type == 2)
    {
        //seek to the start of the data     why????
    ...
    else if (header.type == 10)//run length encoded, non color mapped rgb
    {
        ...
        c = 0; r = 0;//start at the top left
        //work through the bitmap
        while (r < header.height)
        {
            ...
            for (i = 0; i < n + 1; i++)
            {
                ...
                if (bytespp == 4 || bytespp == 3)
                {
                    pixels[(c + (header.height - r - 1)*header.width)*bytespp + 0] = pixel[2];
                    pixels[(c + (header.height - r - 1)*header.width)*bytespp + 1] = pixel[1];
                    pixels[(c + (header.height - r - 1)*header.width)*bytespp + 2] = pixel[0];
                }
                if (bytespp==4)
                    pixels[(c + (header.height - r - 1)*header.width)*bytespp + 3] = pixel[3];
